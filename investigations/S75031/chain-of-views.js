...
// Run with: ~/bin/mongosh-1.8.0-darwin-arm64/bin/mongosh ./chain-of-views.js
/**
...
// with the QE-encrypted field renamed.
encryptedDb.createCollection("view1", {
    viewOn: QE_COLLECTION_NAME,
    pipeline: [{
        $project: {
            renamedEncryptedField: "$myEncryptedField"
        }
    }]
});

// Use the encrypted client to create a view of the view with matching the renamedEncryptedField.
encryptedDb.createCollection("view2", {
    viewOn: "view1",
    pipeline: [{
        $match: {
            renamedEncryptedField: 3
        }
    }]
});
