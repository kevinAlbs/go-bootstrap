// To run this example:
// 	 go run -tags cse ./csfle/fle2_encryptedFieldsMap
...
func logCommandStarted(context context.Context, event *event.CommandStartedEvent) {
	...
	if err != nil {
		log.Fatalf("error in MarshalExtJSONIndent: %v", err)
	}
...
func main() {
	...
	// Drop data from previous runs.
	{
		...
		if err != nil {
			log.Fatalf("error dropping database db: %v", err)
		}
	}
	...
	var keyID primitive.Binary
	{
		...
		if err != nil {
			log.Fatalf("error in NewClientEncryption: %v", err)
		}
		keyID, err = ce.CreateDataKey(context.Background(), "local")
		if err != nil {
			log.Fatalf("error in CreateDataKey: %v", err)
		}
	}
	...
	{
		_, err = encryptedColl.InsertOne(context.Background(), bson.M{"_id": 1, "encryptedIndexed": "foo"})
		if err != nil {
	...
	// Use an unencrypted client to check data was encrypted.
	{
		...
		if res.Err() != nil {
			log.Fatalf("error in FindOne: %v", res.Err())
		}
		raw, err := res.DecodeBytes()
		if err != nil {
			log.Fatalf("error in DecodeBytes: %v", err)
		}
		gotType := raw.Lookup("encryptedIndexed").Type
		if gotType != bsontype.Binary {
			log.Fatalf("expected Binary data, got: %v", gotType)
		}
	}
	...
	// Use a client with encrypted field map with a missing keyId.
	{
		...
		aeOpts := options.AutoEncryption().
			SetKeyVaultNamespace("db.datakeys").
			SetKmsProviders(kmsProviders).
			SetEncryptedFieldsMap(efm)

		co := options.Client().ApplyURI(uri).SetAutoEncryptionOptions(aeOpts)
		if showCmdStarted := os.Getenv("SHOW_COMMAND_STARTED"); showCmdStarted == "ON" {
			co.SetMonitor(&event.CommandMonitor{Started: logCommandStarted})
		}
		encryptedClientEFM, err := mongo.Connect(context.Background(), co)
		defer encryptedClientEFM.Disconnect(context.Background())
		if err != nil {
			log.Fatalf("error in Connect: %v", err)
		}

		encryptedCollEFM := encryptedClientEFM.Database("db").Collection("encrypted")
